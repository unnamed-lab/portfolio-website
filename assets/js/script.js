...
//  Experience Section
const expBody = document.querySelector(".work-tree");
const expList = [
  {
...
(function () {
  ...
  expList.map((item) => {
    ...
    const html = `
<div class="branch">
  <div class="timespan">${item.timespan}</div>
  <div class="content">
    <div class="header">
      ${item.title}
    </div>
    <div class="branch-info">
      <div class="tablet type-${typeID}">${item.type}</div>
      <div class="tablet location">${item.location}</div>
    </div>
    <div class="brief">
    ${item.info}
    </div>
    ...
    expBody.insertAdjacentHTML("beforeend", html);
  });
})();
...
const catalogueList = [
  ...
  },
  {
    ...
    info: "Lorem ipsum dolor sit amet consectetur, adipisicing elit. Iure similique modi alias pariatur reprehenderit? Reprehenderit voluptatem dolorum laboriosam, nam illum saepe corporis nobis iusto.",
    url: "https://unnamed-portfolio-build.vercel.app/",
    stacks: ["HTML", "CSS", "JavaScript"],
  },
  {
    id: 2,
    type: 2,
    title: "Dummy Website Build",
    info: "Lorem ipsum dolor sit amet consectetur, adipisicing elit. Iure similique modi alias pariatur reprehenderit? Reprehenderit voluptatem dolorum laboriosam, nam illum saepe corporis nobis iusto.",
    url: "https://unnamed-web3-build01.vercel.app/",
    stacks: ["HTML", "CSS", "JavaScript", "PHP"],
  },
];
const insertHTML = (item) => {
  const stackItem = item.stacks
    .map((element) => `<li class="stack-item">${element}</li>`)
    .join("");
  const html = `
...
const filterProject = function (typeID) {
  catalogueBody.innerHTML = "";
  catalogueList
    .filter((el) => el.type === typeID)
    .map((item) => insertHTML(item));
};
const btnProjectAll = document.querySelector("#projectAll");
const btnProjectWeb = document.querySelector("#projectWeb");
const btnProjectApp = document.querySelector("#projectApp");
const btnProjectAPI = document.querySelector("#projectAPI");
btnProjectAll.addEventListener("click", function () {
  catalogueBody.innerHTML = "";
  catalogueList.map((item) => insertHTML(item));
});
btnProjectWeb.addEventListener("click", function () {
  filterProject(0);
});
btnProjectApp.addEventListener("click", function () {
  filterProject(1);
});
btnProjectAPI.addEventListener("click", function () {
  filterProject(2);
});
(function () {
  catalogueBody.innerHTML = "";
